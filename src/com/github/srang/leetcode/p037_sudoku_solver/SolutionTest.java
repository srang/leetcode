package com.github.srang.leetcode.p037_sudoku_solver;

import com.github.srang.leetcode.helper.SodukoBoard;
import org.junit.jupiter.api.Test;

import static org.junit.jupiter.api.Assertions.assertEquals;

public class SolutionTest {
    @Test
    void testSimpleElimination() {
        SodukoBoard input = new SodukoBoard(new char[][]{
                {'5','3','.','.','7','.','.','.','.'},
                {'6','.','.','1','9','5','.','.','.'},
                {'.','9','8','.','.','.','.','6','.'},
                {'8','.','.','.','6','.','.','.','3'},
                {'4','.','.','8','.','3','.','.','1'},
                {'7','.','.','.','2','.','.','.','6'},
                {'.','6','.','.','.','.','2','8','.'},
                {'.','.','.','4','1','9','.','.','5'},
                {'.','.','.','.','8','.','.','7','9'}
        });
        SodukoBoard solution = new SodukoBoard(new char[][]{
                {'5','3','4','6','7','8','9','1','2'},
                {'6','7','2','1','9','5','3','4','8'},
                {'1','9','8','3','4','2','5','6','7'},
                {'8','5','9','7','6','1','4','2','3'},
                {'4','2','6','8','5','3','7','9','1'},
                {'7','1','3','9','2','4','8','5','6'},
                {'9','6','1','5','3','7','2','8','4'},
                {'2','8','7','4','1','9','6','3','5'},
                {'3','4','5','2','8','6','1','7','9'}
        });
        Solution.solveSudoku(input.getBoard());
        assertEquals(solution.toString(), input.toString());
    }
    @Test
    void testLastRemainingRow() {
        SodukoBoard input = new SodukoBoard(new char[][]{
                {'5','.','4','6','7','8','9','1','2'},
                {'6','7','2','1','9','5','3','4','8'},
                {'1','9','8','3','4','2','5','6','7'},
                {'8','5','9','7','6','1','4','2','3'},
                {'4','2','6','8','5','3','7','9','1'},
                {'7','1','3','9','2','4','8','5','6'},
                {'9','6','1','5','3','7','2','8','4'},
                {'2','8','7','4','1','9','6','3','5'},
                {'3','4','5','2','8','6','1','7','9'}
        });
        SodukoBoard solution = new SodukoBoard(new char[][]{
                {'5','3','4','6','7','8','9','1','2'},
                {'6','7','2','1','9','5','3','4','8'},
                {'1','9','8','3','4','2','5','6','7'},
                {'8','5','9','7','6','1','4','2','3'},
                {'4','2','6','8','5','3','7','9','1'},
                {'7','1','3','9','2','4','8','5','6'},
                {'9','6','1','5','3','7','2','8','4'},
                {'2','8','7','4','1','9','6','3','5'},
                {'3','4','5','2','8','6','1','7','9'}
        });
        Solution.solveSudoku(input.getBoard());
        assertEquals(solution.toString(), input.toString());
    }

    @Test
    void test03() {
        SodukoBoard input = new SodukoBoard(new char[][]{
                {'5','.','4','6','7','.','9','1','2'},
                {'6','7','2','1','9','5','3','4','8'},
                {'1','9','8','3','.','2','5','6','7'},
                {'8','5','9','7','6','1','4','2','3'},
                {'4','2','6','8','5','3','7','9','1'},
                {'7','1','3','9','2','4','8','5','6'},
                {'9','6','1','5','.','7','2','8','4'},
                {'2','8','7','4','1','9','6','3','5'},
                {'.','.','.','2','8','6','1','7','9'}
        });
        SodukoBoard solution = new SodukoBoard(new char[][]{
                {'5','3','4','6','7','8','9','1','2'},
                {'6','7','2','1','9','5','3','4','8'},
                {'1','9','8','3','4','2','5','6','7'},
                {'8','5','9','7','6','1','4','2','3'},
                {'4','2','6','8','5','3','7','9','1'},
                {'7','1','3','9','2','4','8','5','6'},
                {'9','6','1','5','3','7','2','8','4'},
                {'2','8','7','4','1','9','6','3','5'},
                {'3','4','5','2','8','6','1','7','9'}
        });
        Solution.solveSudoku(input.getBoard());
        assertEquals(solution.toString(), input.toString());
    }

    @Test
    void test04() {
        SodukoBoard input = new SodukoBoard(new char[][]{
                {'1','.','.','.','.','6','8','3','.'},
                {'.','8','.','5','7','3','.','9','.'},
                {'.','.','.','.','2','8','.','.','.'},
                {'.','7','.','3','.','1','9','8','.'},
                {'5','4','9','.','8','7','6','1','.'},
                {'8','.','.','4','.','9','2','.','.'},
                {'.','.','.','.','.','2','3','.','.'},
                {'7','6','3','.','.','.','.','.','9'},
                {'9','2','8','7','.','.','5','.','.'},
        });
        SodukoBoard solution = new SodukoBoard(new char[][]{
                {'1','5','7','9','4','6','8','3','2'},
                {'2','8','4','5','7','3','1','9','6'},
                {'3','9','6','1','2','8','7','4','5'},
                {'6','7','2','3','5','1','9','8','4'},
                {'5','4','9','2','8','7','6','1','3'},
                {'8','3','1','4','6','9','2','5','7'},
                {'4','1','5','6','9','2','3','7','8'},
                {'7','6','3','8','1','5','4','2','9'},
                {'9','2','8','7','3','4','5','6','1'},
        });
        Solution.solveSudoku(input.getBoard());
        assertEquals(solution.toString(), input.toString());
    }


    @Test
    void test05() {
        SodukoBoard input = new SodukoBoard(new char[][]{
                {'.','.','9','7','4','8','.','.','.'},
                {'7','.','.','.','.','.','.','.','.'},
                {'.','2','.','1','.','9','.','.','.'},
                {'.','.','7','.','.','.','2','4','.'},
                {'.','6','4','.','1','.','5','9','.'},
                {'.','9','8','.','.','.','3','.','.'},
                {'.','.','.','8','.','3','.','2','.'},
                {'.','.','.','.','.','.','.','.','6'},
                {'.','.','.','2','7','5','9','.','.'}
        });
        SodukoBoard solution = new SodukoBoard(new char[][]{
                {'5','1','9','7','4','8','6','3','2'},
                {'7','8','3','6','5','2','4','1','9'},
                {'4','2','6','1','3','9','8','7','5'},
                {'3','5','7','9','8','6','2','4','1'},
                {'2','6','4','3','1','7','5','9','8'},
                {'1','9','8','5','2','4','3','6','7'},
                {'9','7','5','8','6','3','1','2','4'},
                {'8','3','2','4','9','1','7','5','6'},
                {'6','4','1','2','7','5','9','8','3'}
        });
        Solution.solveSudoku(input.getBoard());
        System.out.printf("Number of unsolved cells: %d\n",
                input.toString().chars().filter(ch -> ch == '.').count());
        assertEquals(solution.toString(), input.toString());
    }

    @Test
    void test06() {
        SodukoBoard input = new SodukoBoard(new char[][]{
                {'.','.','3','2','.','6','8','.','.'},
                {'.','5','1','.','.','.','7','2','.'},
                {'.','.','.','8','7','.','.','4','5'},
                {'.','6','.','.','.','.','9','.','3'},
                {'.','.','.','.','.','.','.','5','.'},
                {'.','4','.','.','1','.','6','.','.'},
                {'.','.','5','1','3','2','.','.','.'},
                {'7','.','.','5','.','.','1','.','.'},
                {'2','1','.','.','.','4','.','.','8'}
        });
        SodukoBoard solution = new SodukoBoard(new char[][]{
                {'4','7','3','2','5','6','8','9','1'},
                {'8','5','1','3','4','9','7','2','6'},
                {'9','2','6','8','7','1','3','4','5'},
                {'5','6','8','4','2','7','9','1','3'},
                {'1','9','7','6','8','3','2','5','4'},
                {'3','4','2','9','1','5','6','8','7'},
                {'6','8','5','1','3','2','4','7','9'},
                {'7','3','4','5','9','8','1','6','2'},
                {'2','1','9','7','6','4','5','3','8'}
        });
        Solution.solveSudoku(input.getBoard());
        System.out.printf("Number of unsolved cells: %d\n",
                input.toString().chars().filter(ch -> ch == '.').count());
        assertEquals(solution.toString(), input.toString());
    }


    @Test
    void test07() {
        SodukoBoard input = new SodukoBoard(new char[][]{
                {'.','2','.','.','5','.','8','.','.'},
                {'1','.','.','.','.','.','.','9','4'},
                {'.','.','.','8','1','9','2','.','.'},
                {'.','.','5','.','.','1','4','.','.'},
                {'.','.','3','.','7','.','.','5','.'},
                {'.','.','.','2','4','5','6','.','.'},
                {'.','.','.','.','.','.','1','2','.'},
                {'.','3','.','.','.','6','5','8','9'},
                {'9','.','.','.','8','3','.','.','.'}
        });
        SodukoBoard solution = new SodukoBoard(new char[][]{
                {'6','2','9','3','5','4','8','1','7'},
                {'1','5','8','7','6','2','3','9','4'},
                {'3','7','4','8','1','9','2','6','5'},
                {'8','6','5','9','3','1','4','7','2'},
                {'2','4','3','6','7','8','9','5','1'},
                {'7','9','1','2','4','5','6','3','8'},
                {'5','8','6','4','9','7','1','2','3'},
                {'4','3','7','1','2','6','5','8','9'},
                {'9','1','2','5','8','3','7','4','6'}
        });
        Solution.solveSudoku(input.getBoard());
        System.out.printf("Number of unsolved cells: %d\n",
                input.toString().chars().filter(ch -> ch == '.').count());
        assertEquals(solution.toString(), input.toString());
    }

    @Test
    void test08() {
        SodukoBoard input = new SodukoBoard(new char[][]{
                {'.','.','5','.','.','8','.','.','.'},
                {'.','.','.','.','7','.','.','8','.'},
                {'.','.','.','3','.','.','4','7','5'},
                {'.','.','.','.','.','7','.','.','.'},
                {'.','.','.','.','9','4','1','.','.'},
                {'.','9','.','8','.','.','.','.','.'},
                {'9','2','.','.','.','.','.','3','8'},
                {'1','.','.','.','.','6','.','.','9'},
                {'3','4','.','.','.','2','6','.','.'}
        });
        SodukoBoard solution = new SodukoBoard(new char[][]{
                {'7','3','5','2','4','8','9','1','6'},
                {'4','6','9','1','7','5','3','8','2'},
                {'2','8','1','3','6','9','4','7','5'},
                {'5','1','3','6','2','7','8','9','4'},
                {'8','7','2','5','9','4','1','6','3'},
                {'6','9','4','8','1','3','5','2','7'},
                {'9','2','6','4','5','1','7','3','8'},
                {'1','5','8','7','3','6','2','4','9'},
                {'3','4','7','9','8','2','6','5','1'}
        });
        Solution.solveSudoku(input.getBoard());
        System.out.printf("Number of unsolved cells: %d\n",
                input.toString().chars().filter(ch -> ch == '.').count());
        assertEquals(solution.toString(), input.toString());
    }

    @Test
    void test09() {
        SodukoBoard input = new SodukoBoard(new char[][]{
                {'.','.','.','.','.','1','.','4','.'},
                {'.','.','1','.','.','9','.','.','6'},
                {'4','7','.','.','.','.','8','.','.'},
                {'.','.','.','1','.','.','4','7','.'},
                {'7','.','.','5','9','8','.','.','.'},
                {'.','1','.','7','.','.','.','3','.'},
                {'.','.','.','.','.','3','.','.','.'},
                {'.','.','2','.','.','.','.','.','.'},
                {'5','.','8','4','.','.','9','.','.'}
        });
        SodukoBoard solution = new SodukoBoard(new char[][]{
                {'9','2','6','8','7','1','3','4','5'},
                {'8','5','1','3','4','9','7','2','6'},
                {'4','7','3','2','5','6','8','9','1'},
                {'6','8','5','1','3','2','4','7','9'},
                {'7','3','4','5','9','8','1','6','2'},
                {'2','1','9','7','6','4','5','3','8'},
                {'1','9','7','6','8','3','2','5','4'},
                {'3','4','2','9','1','5','6','8','7'},
                {'5','6','8','4','2','7','9','1','3'}
        });
        Solution.solveSudoku(input.getBoard());
        System.out.printf("Number of unsolved cells: %d\n",
                input.toString().chars().filter(ch -> ch == '.').count());
        assertEquals(solution.toString(), input.toString());
    }

    @Test
    void test10() {
        SodukoBoard input = new SodukoBoard(new char[][]{
                {'3','.','5','.','.','.','.','7','.'},
                {'7','.','.','.','.','1','.','.','.'},
                {'.','.','.','4','.','.','.','.','6'},
                {'.','6','.','.','.','.','5','9','8'},
                {'4','.','.','.','8','.','.','.','.'},
                {'.','3','.','2','.','.','.','.','.'},
                {'.','.','3','.','.','.','.','2','7'},
                {'.','.','.','.','4','.','.','.','.'},
                {'.','5','.','1','9','.','.','8','.'}
        });
        SodukoBoard solution = new SodukoBoard(new char[][]{
                {'9','2','6','8','7','1','3','4','5'},
                {'8','5','1','3','4','9','7','2','6'},
                {'4','7','3','2','5','6','8','9','1'},
                {'6','8','5','1','3','2','4','7','9'},
                {'7','3','4','5','9','8','1','6','2'},
                {'2','1','9','7','6','4','5','3','8'},
                {'1','9','7','6','8','3','2','5','4'},
                {'3','4','2','9','1','5','6','8','7'},
                {'5','6','8','4','2','7','9','1','3'}
        });
        Solution.solveSudoku(input.getBoard());
        System.out.printf("Number of unsolved cells: %d\n",
                input.toString().chars().filter(ch -> ch == '.').count());
        assertEquals(solution.toString(), input.toString());
    }

}
